...
import frc.robot.BreakerLib.subsystemcores.drivetrain.swerve.swervemodules.MK4iSwerveModule;
import frc.robot.BreakerLib.subsystemcores.drivetrain.swerve.swervemodules.GenericSwerveModule;
import frc.robot.BreakerLib.util.BreakerRoboRIO;
import frc.robot.BreakerLib.util.math.BreakerMath;
import frc.robot.BreakerLib.util.math.BreakerUnits;
import frc.robot.BreakerLib.util.selftest.DeviceHealth;

public class SwerveDrive implements GenericDrivetrain, BreakerGenericDevice, GenericOdometer {
  ...
  private GenericSwerveModule[] swerveModules;

  private BreakerPigeon2 pigeon2;

  private SwerveDriveOdometry odometer;

  private MovementState2d prevMovementState = new MovementState2d();
  private MovementState2d curMovementState = new MovementState2d();
   ...
   * 
   * @param config        - the confiuration values for the drivetrain's
   *                      charicotristics and behavor, passed in as a
   *                      "BreakerSwerveDriveConfig" object
   * @param swerveModules - The four swerve drive modules that make up the
   *                      drivetrain, must be passed in the same order shown below
   */
  public SwerveDrive(SwerveDriveConfig config, BreakerPigeon2 pigeon2,
      GenericSwerveModule... swerveModules) {
  ...
  public void moveRelativeToField(double forwardVelMetersPerSec, double horizontalVelMetersPerSec, double radPerSec) {
    ChassisSpeeds robotRelSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(forwardVelMetersPerSec,
        horizontalVelMetersPerSec, radPerSec, getOdometryPoseMeters().getRotation());
    move(robotRelSpeeds);
  }
   ...
   */
  public void moveRelativeToField(double forwardVelMetersPerSec, double horizontalVelMetersPerSec, double radPerSec,
      GenericOdometer odometer) {
    ChassisSpeeds robotRelSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(forwardVelMetersPerSec,
        horizontalVelMetersPerSec, radPerSec, odometer.getOdometryPoseMeters().getRotation());
    move(robotRelSpeeds);
  }
  ...
  public void updateOdometry() {
    odometer.updateWithTime(Timer.getFPGATimestamp(), Rotation2d.fromDegrees(pigeon2.getRawAngles()[0]),
  ...
  public void runSelfTest() {
    faults = null;
    for (GenericSwerveModule module : swerveModules) {
