...
import frc.robot.BreakerLib.position.odometry.differential.DiffDriveState;
...
public class DiffDrive implements GenericDrivetrain, BreakerGenericDevice, GenericOdometer {
  ...
  public DiffDrive(WPI_TalonFX[] leftMotors, WPI_TalonFX[] rightMotors, boolean invertL, boolean invertR,
      BreakerPigeon2 pigeon2, DiffDriveConfig driveConfig) {
  ...
  public DiffDriveState getDiffDriveState() {
    return new DiffDriveState(getWheelSpeeds(), getLeftDriveMeters(), getRightDriveMeters());
  }
  ...
  private void calculateMovementState(double timeToLastUpdateMilisecods) {
    MovementState2d curMovementState = BreakerMath.movementStateFromChassisSpeedsAndPreviousState(
