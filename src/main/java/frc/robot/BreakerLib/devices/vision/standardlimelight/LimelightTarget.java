...
public class LimelightTarget {
   ...
   * 
   * @param targetHeight the height of the intended target form the ground (refers
   *                     to the vision target being tracked)
   * @param limelight    the limelight used to track the intended target (refers
   *                     to the vision target being tracked)
   * @param pipelineNum  the pipeline number (0 - 9) that the limelight needs to
   *                     use to track the intended target
   */
  public LimelightTarget(double targetHeight, Limelight limelight, double pipelineNum) {
  ...
  public double getTargetOffsetY() {
    ...
    return (targetOffsetHasBeenSet ? (Math.toDegrees(Math.atan((newHeight / getTargetDistance()))))
        : getRawTargetOffsetY());
  }
  ...
  public double getTargetDistance() {
    return (targetOffsetHasBeenSet
        ? (BreakerMath.getHypotenuse((getRawTargetDistance() + targetOffsetDistZ), targetOffsetDistX))
        : getRawTargetDistance());
  }
}
