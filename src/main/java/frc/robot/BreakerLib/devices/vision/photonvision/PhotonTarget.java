 ...
 * camera.
 */
public class PhotonTarget extends SubsystemBase {

    private PhotonCamera camera;
    private double targetHeightInches;
    private GenericOdometer odometryProveider;
     ...
     * @param odometryProvider                   BreakerGenericOdometer such as
     *                                           drivetrain or pose estimator for
     *                                           position
     *                                           tracking.
     ...
     */
    public PhotonTarget(PhotonCamera camera, GenericOdometer odometryProveider,
            Pose3d targetPosition, double maxTargetCoordinateDeviationInches) {
        this.camera = camera;
     ...
     */
    public PhotonTarget(PhotonCamera camera, Supplier<PhotonTrackedTarget> assignedTargetSupplier,
    ...
    private void findAssignedTarget() {
        ...
        if (!targetPreAssigned && camera.hasTargets()) {
            ...
            for (PhotonTrackedTarget prospTgt : camera.getAllRawTrackedTargets()) {
                ...
                Pose2d prospTgtPose = odometryProveider.getOdometryPoseMeters().transformBy(prospTgtTransform); // 2d
                                                                                                                // target
                // position
                // relative to
                // field.
